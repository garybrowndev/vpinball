...
- `master` — tracks upstream `vpinball/vpinball` **plus two local patches** (see "Master is not pure upstream" below).
...
Both were written months earlier against upstream code that has since been rewritten, and both kept replaying through rebases unexamined. **A patch that applies cleanly is not evidence that it is still needed** — re-test the original symptom periodically rather than assuming.

**"Fix Debug builds hanging when loading VPinMAME tables with .NET COM servers" (#24)** set `StackReserveSize` 0 → 1048576 in `make/vpx-core.vcxitems`. That file belongs to the **deprecated VS-solution build**, which CMake never reads, and the config names it patched (`Debug_GL`, `Debug_BGFX`) don't exist under CMake at all. Verified against CI artifacts: the CMake Release **and Debug** exes already report `100000` stack reserve / `1000` commit — MSVC's defaults — so the hang cannot occur on the build we use.

**"Fix InputManager crash when multiple devices share a settings ID"** removed two asserts in `RegisterDevice`/`UnregisterDevice`. Upstream commit `a7c29b1be` deleted `GetJoySettingId` (which the patch's own comment cited) and replaced the settings-id scheme with one that appends a per-GUID ordinal — `SDLInputHandler.h:226-242` — so identical devices no longer collide. Verified empirically: a Debug build with the patch reverted, three identical DragonRise encoders connected, registered all four devices cleanly and reached `Player::Player@832 Startup done` with no assert. The original crash was at exactly that point.

Both were dropped from **master only**. `integration` retains the changed file content, inherited through an earlier merge — a dropped commit does not propagate through a 3-way merge. That residue is deliberate: both files also carry Ball History changes (`vpx-core.vcxitems` has `__BALLHISTORY_WIN32__`; `InputManager.cpp` has the V/R key action registration), so restoring either wholesale from upstream would destroy them. The residue is inert — dead build file in one case, unreachable collision handling in the other.
...
git fetch upstream
git checkout master
git rebase upstream/master       # replays both patches on top of the new upstream tip
...
**After any rebase, verify both survived** — `git log upstream/master..master --oneline --no-merges` should list all of them, and comparing each patch's added/removed lines against the pre-rebase version catches a mis-replay that a bare commit count would miss.
